...
- **动态页热搜「屏蔽话题」**：动态页（t.bilibili.com）右侧 bilibili 热搜 hover 出“屏蔽”，点一下用 AI 把整条热搜抽成几个关键词加进屏蔽词，连带过滤掉推荐流里同话题的视频。
...
- 在动态页右侧热搜上点“屏蔽”，AI 会把这条热搜抽成关键词加进屏蔽列表，同话题的视频以后都不再刷到（抽出来的词可在关键词 tab 里查看 / 删除）。
- 在画像 tab 看 AI 总结出来的 interests、disinterests 和分析文字，不准的标签可以直接改掉。

“不感兴趣”和“不看TA”都会算进行为计数，攒够阈值时 background 会重新跑一次 LLM 分析。
...
- **行为触发分析**：“不感兴趣”和“不看TA”都会算进行为计数，攒够阈值时 background 重新跑一次 LLM 分析。
