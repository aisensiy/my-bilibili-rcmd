...
用户删过的词（他主动移除的屏蔽词＝明确表态"这类内容我不想被屏蔽"，可能是之前误伤了他爱看的、也可能是他改了主意。绝对不要再把它们输出为屏蔽词，并据此校正画像，见下）：
...
注意：
- interests / disinterests 是给用户看的"画像镜子"：概念化的喜欢/不喜欢类型（如"科技"、"营销号内容"），不要求能匹配标题。生成 disinterests 时要参考上面"用户删过的词"做校正：若某条不喜欢概念正好就是某个被删词所代表的（如他删了"乒乓球"、画像里却有"不喜欢乒乓球"），说明他对这条改了主意，应去掉该概念；但若被删的词只是某个更宽概念下一个不准的提法（如删了"速通"而真正不喜欢的是更上位的"营销号教程"），那只是这个词不合适，保留该概念、别动。
- blockedUps 来自"不看TA"行为，直接取 upName。
- disinterestKeywords 这次只输出"本轮新发现的候选词"（最多 10 个），用于"标题子串匹配"，要表达【当前画像 disinterests 里、这个用户自己不喜欢的方向】。不要预设任何"什么内容算垃圾"，一切以这个用户的喜欢/不喜欢为准。规则：
  - 字面溯源：每个词必须是【不想看】或【刷到的】里某条标题/话题"确实出现过的连续字面片段"，不要凭空造词。
  - 表达"某一类不喜欢的内容"、可泛化：挑那种"换一条同类、用户也会不喜欢的视频，标题里还很可能出现"的说法；不要只描述某一条视频独有的专名（具体人名/作品名/事件名）。
  - 不误伤（最关键的判据）：对每个候选词逐条对照【爱看】——只要它可能出现在任何一条【爱看】标题里，就丢掉。同一主题里往往既有用户讨厌的也有他爱看的，能区分的只有"这个词会不会也落在他爱看的内容上"。也不要输出"已生效的屏蔽词"和"黑名单"里的词。没把握就给空数组。
  - 不要无差别的网络流行语/弹幕梗/口头禅——这类词在喜欢和不喜欢的内容里都出现，必然误伤。`
}
