...
async function buildProfile(): Promise<void> {
  ...
  const model: string = settings.model || 'google/gemini-2.0-flash-exp:free'

  if (!apiKey) {
    console.log('[BiliFilter] No API key configured, skipping analysis')
    return
  }

  // Use recent 50 actions for analysis
...
- 标签要具体，方便后续关键词匹配（比如"军事"而不是"严肃内容"）`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[BiliFilter] OpenRouter error:', response.status, err)
      return
    }
    ...
    console.error('[BiliFilter] Failed to build profile:', err)
  }
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === 'check_trigger') {
    storageGet(['actionsSinceLastAnalysis', 'settings']).then(({ actionsSinceLastAnalysis = 0, settings = {} }) => {
      const threshold = settings.triggerThreshold ?? 10
